    ...
    private fun decodeRevisionRequest(ctx: ChannelHandlerContext, buf: ByteBuf) {
        if(buf.readableBytes() >= 4) {
            ...
            if(revision != serverRevision) {
                logger.info("Revision mismatch for channel {} with client revision {}. Client must be using a revision {} gamepack to request.", ctx.channel(), revision, serverRevision)
